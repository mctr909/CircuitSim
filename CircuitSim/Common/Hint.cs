...
namespace Circuit {
    class Hint {
        ...
        static BaseUI getElm(List<BaseUI> elmList, int n) {
            if (n >= elmList.Count) {
                return null;
            }
            return elmList[n];
        ...
        public static string getHint(List<BaseUI> elmList) {
            ...
            if (Type == HINT_LC) {
                ...
                if (!(c2 is CapacitorUI)) {
                    return null;
                }
                var ie = (InductorElm)c1.CirElm;
                var ce = (CapacitorElm)c2.CirElm;

...
namespace Circuit.Elements.Passive {
    class CrystalElm : CompositeElm {
        ...
        public override void Draw() {
            ...
            Context.DrawLine(mPlate1[0], mPlate1[1]);

            // draw second lead and plate
            drawVoltage(1, mPoint2, mLead2);
            Context.DrawLine(mPlate2[0], mPlate2[1]);

            Context.LineColor = getVoltageColor(0.5 * (Volts[0] + Volts[1]));
            for (int i = 0; i != 4; i++) {
                Context.DrawLine(mSandwichPoints[i], mSandwichPoints[(i + 1) % 4]);
            }

...
namespace MainForm {
	public partial class MainForm : Form {
		...
		static void UpdateDisplay() {
			...
			if (MouseInfo.GrippedElm == null) {
				var max = 0.01 * Math.Sqrt(Math.Max(
					Math.Max(CircuitState.ElapseTimeElm, CircuitState.ElapseTimeLU),
					CircuitState.ElapseTimeVal
				));
				if (0 == max) {
					max = 1;
				}
				ControlPanel.LblSelectInfo.Text
					= $"反復回数:{CircuitState.SubIterationCount:f0}\r\n"
					+ $"LU:{Math.Sqrt(CircuitState.ElapseTimeLU)*1000:f0}μsec\r\n"
					+ $"Elm:{Math.Sqrt(CircuitState.ElapseTimeElm)/max:f0}%\r\n"
					+ $"SetVA:{Math.Sqrt(CircuitState.ElapseTimeVal)/max:f0}%";
			} else {

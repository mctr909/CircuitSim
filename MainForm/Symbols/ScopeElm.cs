namespace MainForm;

internal class ScopeElm : ElmBase {
	public Scope elmScope;

	public ScopeElm(int xx, int yy) : base(xx, yy) {
		noDiagonal = false;
		x2 = x + 128;
		y2 = y + 64;
		elmScope = new Scope(sim);
		setPoints();
	}

	public ScopeElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) : base(xa, ya, xb, yb, f) {
		noDiagonal = false;
		var sStr = st.nextToken();
		var sst = new StringTokenizer(sStr, "_");
		elmScope = new Scope(sim);
		elmScope.undump(sst);
		setPoints();
		elmScope.resetGraph();
	}

	public void setScopeElm(ElmBase e) {
		elmScope.setElm(e);
		elmScope.resetGraph();
	}

	public void setScopeRect() {
		var i1 = sim.transformX(Math.Min(x, x2));
		var i2 = sim.transformX(Math.Max(x, x2));
		var j1 = sim.transformY(Math.Min(y, y2));
		var j2 = sim.transformY(Math.Max(y, y2));
		var r = new Rectangle(i1, j1, i2 - i1, j2 - j1);
		if (!r.Equals(elmScope.rect))
			elmScope.setRect(r);
	}

	public override void setPoints() {
		base.setPoints();
		setScopeRect();
	}

	public void setElmScope(Scope s) {
		elmScope = s;
	}

	public void stepScope() {
		elmScope.timeStep();
	}

	public override void reset() {
		base.reset();
		elmScope.resetGraph(true);
	}

	public void clearElmScope() {
		elmScope = null;
	}

	public override bool canViewInScope() {
		return false;
	}

	public override int getDumpType() {
		return 403;
	}

	public override string dump() {
		var dumpStr = base.dump();
		var sStr = elmScope.dump().Replace(' ', '_');
		sStr = sStr[2..]; // remove unused prefix(o_) for embedded Scope
		return dumpStr + " " + sStr;
	}

	public override void draw(CustomGraphics g) {
		g.setColor(needsHighlight() ? selectColor : whiteColor);
		g.save();
		g.transform(1, 0, 0, 1, 0, 0);
		setScopeRect();
		elmScope.draw(g);
		g.restore();
		setBbox(point1, point2, 0);
		drawPosts(g);
	}

	public override int getPostCount() {
		return 0;
	}
}

...
namespace Circuit {
	static class CircuitElement {
		...
		public static int NODE_COUNT { get; private set; }
		...
		static bool DoIteration() {
			...
			for (ITER_COUNT = 0; ITER_COUNT < ITER_LIMIT; ITER_COUNT++) {
				...
				var msecBegin = mSw.ElapsedMilliseconds;
				CircuitState.Converged = true;
				for (int i = 0; i < m_element_count; i++) {
					m_elements[i].DoIteration();
				}
				var diffTick = mSw.ElapsedMilliseconds - msecBegin;
				CircuitState.ElapseTimeElm += (diffTick*diffTick - CircuitState.ElapseTimeElm) * 0.002;
				...
				msecBegin = mSw.ElapsedMilliseconds;
				LUFactor();
				LUSolve();
				diffTick = mSw.ElapsedMilliseconds - msecBegin;
				CircuitState.ElapseTimeLU += (diffTick*diffTick - CircuitState.ElapseTimeLU) * 0.002;

				msecBegin = mSw.ElapsedMilliseconds;
				var termNode = NODE_COUNT - 1;
				...
				diffTick = mSw.ElapsedMilliseconds - msecBegin;
				CircuitState.ElapseTimeVal += (diffTick*diffTick - CircuitState.ElapseTimeVal) * 0.002;
			}

			CircuitState.SubIterationCount *= 0.999;
			if (ITER_COUNT > CircuitState.SubIterationCount) {
				CircuitState.SubIterationCount = ITER_COUNT;
			}

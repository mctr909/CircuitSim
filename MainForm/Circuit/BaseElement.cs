namespace Circuit {
	public abstract class BaseElement {
		...
		#region [method(Stamp)]
		public static void StampMatrix(int r, int c, double val) {
			if (r > 0 && c > 0) {
		...
		public virtual int GetConnection(int n) { return NodeId[n]; }
		...
		public virtual void SetNode(int n, int id) {
			if (n < NodeId.Length) {
				NodeId[n] = id;
			}
		}
		...
		protected static void UpdateVoltage(int vs_index, double v) {
			vs_index += CircuitElement.NODE_COUNT;
			vs_index = CircuitElement.NODE_INFOS[vs_index - 1].row;
			CircuitElement.RIGHT_SIDE[vs_index] += v;
		}

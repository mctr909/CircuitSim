...
namespace Circuit.Symbol.Passive {
	class Resistor : BaseSymbol {
		...
		protected override void dump(List<object> optionList) {
			optionList.Add(Element.Params[0].ToString("g3"));
		}

		public override void Stamp() {
			StampResistor(Element.Nodes[0], Element.Nodes[1], Element.Params[0]);
		}
		...
		public override void Draw(CustomGraphics g) {
			...
			DrawName();
			DrawValue(TextUtils.Unit(Element.Params[0]));
		...
		public override void GetInfo(string[] arr) {
			if (string.IsNullOrEmpty(ReferenceName)) {
				arr[0] = "抵抗：" + TextUtils.Unit(Element.Params[0], "Ω");
				GetBasicInfo(1, arr);
			} else {
				arr[0] = ReferenceName;
				arr[1] = "抵抗：" + TextUtils.Unit(Element.Params[0], "Ω");
				GetBasicInfo(2, arr);
			}
		}
		...
		public override ElementInfo GetElementInfo(int r, int c) {
			...
			if (r == 0) {
				return new ElementInfo("レジスタンス(Ω)", Element.Params[0]);
			}
			if (r == 1) {
		...
		public override EventHandler CreateSlider(ElementInfo ei, Slider adj) {
			return new EventHandler((s, e) => {
				...
				Element.Params[0] = adj.MinValue + (adj.MaxValue - adj.MinValue) * trb.Value / trb.Maximum;

...
namespace Circuit.Symbol.Passive {
	class CapacitorPolar : Capacitor {
		...
		public CapacitorPolar(Point pos) : base(pos) {
			Element.Params[ElmCapacitor.MAX_NEGATIVE] = 5;
		...
		public CapacitorPolar(Point p1, Point p2, int f, StringTokenizer st) : base(p1, p2, f) {
			...
			Element.Params[ElmCapacitor.MAX_VOLTAGE] = 1e12;
			Element.Params[ElmCapacitor.MAX_NEGATIVE] = st.nextTokenDouble();
		}
		...
		protected override void dump(List<object> optionList) {
			...
			optionList.Add(Element.Params[ElmCapacitor.MAX_NEGATIVE]);
		}
		...
		public override ElementInfo GetElementInfo(int r, int c) {
			...
			if (r == 2) {
				return new ElementInfo("耐逆電圧", Element.Params[ElmCapacitor.MAX_NEGATIVE]);
			}
		...
		public override void SetElementValue(int n, int c, ElementInfo ei) {
			if (n == 2 && ei.Value >= 0) {
				Element.Params[ElmCapacitor.MAX_NEGATIVE] = ei.Value;
			}

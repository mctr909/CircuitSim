...
namespace Circuit.Symbol.Active {
	class OpAmp : BaseSymbol {
		...
		public OpAmp(Point pos) : base(pos) {
			...
			Element.Params[ElmOpAmp.OUT_MAX] = 15;
			Element.Params[ElmOpAmp.OUT_MIN] = -15;
			Element.Params[ElmOpAmp.GAIN] = 1e3;
			Post.NoDiagonal = true;
		}

		public OpAmp(Point p1, Point p2, int f, StringTokenizer st) : base(p1, p2, f) {
			Element.Params[ElmOpAmp.OUT_MAX] = st.nextTokenDouble();
			Element.Params[ElmOpAmp.OUT_MIN] = st.nextTokenDouble();
			Element.Params[ElmOpAmp.GAIN] = st.nextTokenDouble();
		...
		protected override void dump(List<object> optionList) {
			mFlags |= FLAG_GAIN;
			optionList.Add(Element.Params[ElmOpAmp.OUT_MAX]);
			optionList.Add(Element.Params[ElmOpAmp.OUT_MIN]);
			optionList.Add(Element.Params[ElmOpAmp.GAIN]);
		...
		public override void GetInfo(string[] arr) {
			arr[0] = "オペアンプ";
			arr[1] = "+電源：" + TextUtils.Voltage(Element.Params[ElmOpAmp.OUT_MAX]);
			arr[2] = "-電源：" + TextUtils.Voltage(Element.Params[ElmOpAmp.OUT_MIN]);
			arr[3] = "Vin(+)：" + TextUtils.Voltage(Element.V[ElmOpAmp.P]);
			arr[4] = "Vin(-)：" + TextUtils.Voltage(Element.V[ElmOpAmp.N]);
			var vo = Math.Max(
				Math.Min(Element.V[ElmOpAmp.O], Element.Params[ElmOpAmp.OUT_MAX]),
				Element.Params[ElmOpAmp.OUT_MIN]
			);
		...
		public override ElementInfo GetElementInfo(int r, int c) {
			...
			if (r == 0) {
				return new ElementInfo("+電源", Element.Params[ElmOpAmp.OUT_MAX]);
			}
			if (r == 1) {
				return new ElementInfo("-電源", Element.Params[ElmOpAmp.OUT_MIN]);
			}
			if (r == 2) {
				return new ElementInfo("ゲイン(db)", 20 * Math.Log10(Element.Params[ElmOpAmp.GAIN]));
			}
			return null;
		}
		...
		public override void SetElementValue(int n, int c, ElementInfo ei) {
			if (n == 0) {
				Element.Params[ElmOpAmp.OUT_MAX] = ei.Value;
			}
			if (n == 1) {
				Element.Params[ElmOpAmp.OUT_MIN] = ei.Value;
			}
			if (n == 2 && ei.Value > 0) {
				Element.Params[ElmOpAmp.GAIN] = Math.Pow(10.0, ei.Value / 20.0);
			}
		}

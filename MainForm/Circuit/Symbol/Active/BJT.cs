...
namespace Circuit.Symbol.Active {
	class BJT : BaseSymbol {
		...
		public BJT(Point pos, bool pnpFlag) : base(pos) {
			Element.Params[ElmBJT.NPN] = pnpFlag ? -1 : 1;
			Element.Params[ElmBJT.MAX_POW] = 1e6;
			ReferenceName = "Tr";
		...
		public BJT(Point p1, Point p2, int f, StringTokenizer st) : base(p1, p2, f) {
			...
			Element.Params[ElmBJT.NPN] = npn;
			Element.Params[ElmBJT.MAX_POW] = pow;
		...
		protected override void dump(List<object> optionList) {
			optionList.Add((int)Element.Params[ElmBJT.NPN]);
			...
			optionList.Add(HFE);
			optionList.Add(Element.Params[ElmBJT.MAX_POW]);
		}
		...
		void SetHfe(double hfe) {
			...
			Element.Params[ElmBJT.GAIN] = gain;
			Element.Params[ElmBJT.GAIN_INV] = 1.0 / gain;
		}
		...
		public override void SetPoints() {
			...
			var hsm = (HS / 8 + 1) * 8;
			var hs1 = (HS - 2) * Post.Dsign * (int)Element.Params[ElmBJT.NPN];
			var hs2 = hsm * Post.Dsign * (int)Element.Params[ElmBJT.NPN];
			...
			InterpolationPostAB(ref mPosC[0], ref mPosE[0],
				...
				5 * Post.Dsign * (int)Element.Params[ElmBJT.NPN]
			);
			...
			/* arrow */
			if (Element.Params[ElmBJT.NPN] > 0) {
				CreateArrow(mPosE[0], mPosE[1], out mArrowPoly, 8, 3);
			} else {
				var b = new PointF();
				InterpolationPost(ref b, 1 - (BODY_LEN - 1) / Post.Len, -5 * Post.Dsign * (int)Element.Params[ElmBJT.NPN]);
		...
		public override void Draw(CustomGraphics g) {
			...
			if (0 <= Element.Params[ElmBJT.NPN] * Element.I[ElmBJT.C]) {
		...
		public override void GetInfo(string[] arr) {
			arr[0] = ((Element.Params[ElmBJT.NPN] < 0) ? "PNP" : "NPN") + "トランジスタ(" + "hfe：" + TextUtils.Unit(HFE) + ")";
			...
			if (vbc * Element.Params[ElmBJT.NPN] > 0.2) {
				arr[1] = "動作領域：" + (vbe * Element.Params[ElmBJT.NPN] > 0.2 ? "飽和" : "逆流");
			} else {
				arr[1] = "動作領域：" + (vbe * Element.Params[ElmBJT.NPN] > 0.2 ? "活性" : "遮断");
			}
		...
		public override ElementInfo GetElementInfo(int r, int c) {
			...
			if (r == 2) {
				return new ElementInfo("許容損失(W)", Element.Params[ElmBJT.MAX_POW]);
			}
			if (r == 3) {
		...
		public override void SetElementValue(int r, int c, ElementInfo ei) {
			...
			if (r == 2) {
				Element.Params[ElmBJT.MAX_POW] = ei.Value;
			}
			if (r == 3) {

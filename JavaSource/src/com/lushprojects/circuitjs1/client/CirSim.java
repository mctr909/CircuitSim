...
public class CirSim implements MouseDownHandler, MouseMoveHandler, MouseUpHandler,
	...
	Vector<ElmBase> elmList;
	Vector<Adjustable> adjustables;
	// Vector setupList;
	ElmBase dragElm, menuElm, stopElm;
	private ElmBase mouseElm = null;
	boolean didSwitch = false;
	int mousePost = -1;
	ElmBase plotXElm, plotYElm;
	int draggingPost;
	...
	public void init() {
		...
		m.addSeparator();
		m.addItem(selectAllItem = menuItemWithShortcut("select-all", LS("Select All"), LS("Ctrl-A"),
				new MyCommand("edit", "selectAll")));
		m.addSeparator();
		m.addItem(iconMenuItem("target", weAreInUS() ? "Center Circuit" : "Centre Circuit",
		...
		elmMenuBar
				.addItem(elmScopeMenuItem = new MenuItem(LS("View in New Scope"), new MyCommand("elm", "viewInScope")));
		elmMenuBar.addItem(elmFloatScopeMenuItem = new MenuItem(LS("View in New Undocked Scope"),
				new MyCommand("elm", "viewInFloatScope")));
		elmMenuBar.addItem(
				elmAddScopeMenuItem = new MenuItem(LS("Add to Existing Scope"), new MyCommand("elm", "addToScope0")));
	...
	void setColors(String positiveColor, String negativeColor) {
		...
		if (positiveColor != null)
			ElmBase.positiveColor = new Color(URL.decodeQueryString(positiveColor));
		else if (getOptionFromStorage("alternativeColor", false))
			ElmBase.positiveColor = Color.blue;
		if (negativeColor != null)
			ElmBase.negativeColor = new Color(URL.decodeQueryString(negativeColor));

		ElmBase.setColorScale();
	}
	...
	CheckboxMenuItem getClassCheckItem(String s, String t) {
		...
		String shortcut = "";
		ElmBase elm = constructElement(t, 0, 0);
		CheckboxMenuItem mi;
		// register(c, elm);
		if (elm != null) {
	...
	public void updateCircuit() {
		...
		CustomGraphics g = new CustomGraphics(backcontext);
		...
		ElmBase.powerMult = Math.exp(powerBar.getValue() / 4.762 - 7);

		int i;
		// Font oldfont = g.getFont();
		Font oldfont = ElmBase.unitsFont;
		g.setFont(oldfont);
		...
		if (mouseMode != CirSim.MODE_DRAG_ROW && mouseMode != CirSim.MODE_DRAG_COLUMN) {
			for (i = 0; i != postDrawList.size(); i++)
				ElmBase.drawPost(g, postDrawList.get(i));
		}
		...
		if (stopMessage != null) {
			...
			if (mouseElm != null) {
				...
				info[1] = LS("time step = ") + ElmBase.getTimeText(timeStep);
			}
			if (hintType != -1) {
	...
	String getHint() {
		ElmBase c1 = getElm(hintItem1);
		ElmBase c2 = getElm(hintItem2);
		if (c1 == null || c2 == null)
			return null;
		if (hintType == HINT_LC) {
			...
			return LS("res.f = ") + ElmBase.getUnitText(1 / (2 * pi * Math.sqrt(ie.inductance *
					ce.capacitance)), "Hz");
		}
		if (hintType == HINT_RC) {
			...
			CapacitorElm ce = (CapacitorElm) c2;
			return "RC = " + ElmBase.getUnitText(re.resistance * ce.capacitance,
					"s");
		}
		if (hintType == HINT_3DB_C) {
			...
			CapacitorElm ce = (CapacitorElm) c2;
			return LS("f.3db = ") +
					ElmBase.getUnitText(1 / (2 * pi * re.resistance * ce.capacitance), "Hz");
		}
		if (hintType == HINT_3DB_L) {
			...
			return LS("f.3db = ") +
					ElmBase.getUnitText(re.resistance / (2 * pi * ie.inductance), "Hz");
		}
	...
	public ElmBase getElm(int n) {
		if (n >= elmList.size())
			return null;
		return elmList.elementAt(n);
	}

	public Adjustable findAdjustable(ElmBase elm, int item) {
		int i;
	...
	boolean calcWireInfo() {
		...
		for (i = 0; i != wireInfoList.size(); i++) {
			...
			Vector<ElmBase> neighbors0 = new Vector<ElmBase>();
			Vector<ElmBase> neighbors1 = new Vector<ElmBase>();
	...
	class FindPathInfo {
		...
		FindPathInfo(int type_, ElmBase elm_, int dest_) {
			dest = dest_;
			type = type_;
			firstElm = elm_;
	...
	public void menuPerformed(String menu, String item) {
		...
		if (item == "fullscreen") {
			if (!CustomGraphics.isFullScreen)
				CustomGraphics.viewFullScreen();
			else
				CustomGraphics.exitFullScreen();
			centreCircuit();
		}
	...
	String dumpCircuit() {
		...
		String dump = "$ " + f + " " +
				...
				currentBar.getValue() + " " + ElmBase.voltageRange + " " +
		...
		for (p = 0; p < len;) {
			...
			while (st.hasMoreTokens()) {
				...
				try {
					...
					ElmBase newce = createCe(tint, x1, y1, x2, y2, f, st);
					if (newce == null) {
	...
	void deleteSliders(ElmBase elm) {
		int i;
		if (adjustables == null)
			return;
	...
	void readOptions(StringTokenizer st) {
		...
		ElmBase.voltageRange = new Double(st.nextToken()).doubleValue();
	...
	void dragPost(int x, int y) {
		if (draggingPost == -1) {
			draggingPost = (CustomGraphics.distanceSq(mouseElm.x, mouseElm.y, x, y) > CustomGraphics.distanceSq(
					mouseElm.x2,
					mouseElm.y2, x, y)) ? 1 : 0;
		}
		int dx = x - dragGridX;
		...
		if (mouseElm != null && (mouseElm.getHandleGrabbedClose(gx, gy, POSTGRABSQ, MINPOSTGRABSIZE) >= 0)) {
			...
			for (i = 0; i != elmList.size(); i++) {
				...
				if (ce.boundingBox.contains(gx, gy)) {
					...
					for (j = 0; j != jn; j++) {
						Point pt = ce.getPost(j);
						int dist = CustomGraphics.distanceSq(gx, gy, pt.x, pt.y);
		...
		if (newMouseElm == null) {
			...
			for (i = 0; i != elmList.size(); i++) {
				...
				for (j = 0; j != jn; j++) {
					...
					if (CustomGraphics.distanceSq(pt.x, pt.y, gx, gy) < 26) {
						newMouseElm = ce;
						mousePost = j;
						break;
					}
	...
	boolean sliderItemEnabled(ElmBase elm) {
		int i;
	...
	public void onMouseDown(MouseDownEvent e) {
		...
		if (!(dialogIsShowing()) && ((scopeSelected != -1 && scopes[scopeSelected].cursorInSettingsWheel()) ||
				(scopeSelected == -1 && mouseElm instanceof ScopeElm
						&& ((ScopeElm) mouseElm).elmScope.cursorInSettingsWheel()))) {
			if (noEditCheckItem.getState())
				return;
			Scope s;
			if (scopeSelected != -1)
	...
	public static ElmBase createCe(int tint, int x1, int y1, int x2, int y2, int f, StringTokenizer st) {
		switch (tint) {
			case 'A':
	...
	public static ElmBase constructElement(String n, int x1, int y1) {
		if (n == "GroundElm")
			return (ElmBase) new GroundElm(x1, y1);
		if (n == "ResistorElm")
			return (ElmBase) new ResistorElm(x1, y1);
		if (n == "RailElm")
			return (ElmBase) new RailElm(x1, y1);
		if (n == "SwitchElm")
			return (ElmBase) new SwitchElm(x1, y1);
		if (n == "Switch2Elm")
			return (ElmBase) new Switch2Elm(x1, y1);
		if (n == "MBBSwitchElm")
			return (ElmBase) new MBBSwitchElm(x1, y1);
		if (n == "NTransistorElm" || n == "TransistorElm")
			return (ElmBase) new NTransistorElm(x1, y1);
		if (n == "PTransistorElm")
			return (ElmBase) new PTransistorElm(x1, y1);
		if (n == "WireElm")
			return (ElmBase) new WireElm(x1, y1);
		if (n == "CapacitorElm")
			return (ElmBase) new CapacitorElm(x1, y1);
		if (n == "PolarCapacitorElm")
			return (ElmBase) new PolarCapacitorElm(x1, y1);
		if (n == "InductorElm")
			return (ElmBase) new InductorElm(x1, y1);
		if (n == "DCVoltageElm" || n == "VoltageElm")
			return (ElmBase) new DCVoltageElm(x1, y1);
		if (n == "VarRailElm")
			return (ElmBase) new VarRailElm(x1, y1);
		if (n == "PotElm")
			return (ElmBase) new PotElm(x1, y1);
		if (n == "OutputElm")
			return (ElmBase) new OutputElm(x1, y1);
		if (n == "CurrentElm")
			return (ElmBase) new CurrentElm(x1, y1);
		if (n == "ProbeElm")
			return (ElmBase) new ProbeElm(x1, y1);
		if (n == "DiodeElm")
			return (ElmBase) new DiodeElm(x1, y1);
		if (n == "ZenerElm")
			return (ElmBase) new ZenerElm(x1, y1);
		if (n == "ACVoltageElm")
			return (ElmBase) new ACVoltageElm(x1, y1);
		if (n == "ACRailElm")
			return (ElmBase) new ACRailElm(x1, y1);
		if (n == "SquareRailElm")
			return (ElmBase) new SquareRailElm(x1, y1);
		if (n == "SweepElm")
			return (ElmBase) new SweepElm(x1, y1);
		if (n == "LEDElm")
			return (ElmBase) new LEDElm(x1, y1);
		if (n == "AntennaElm")
			return (ElmBase) new AntennaElm(x1, y1);
		if (n == "LogicInputElm")
			return (ElmBase) new LogicInputElm(x1, y1);
		if (n == "LogicOutputElm")
			return (ElmBase) new LogicOutputElm(x1, y1);
		if (n == "TransformerElm")
			return (ElmBase) new TransformerElm(x1, y1);
		if (n == "TappedTransformerElm")
			return (ElmBase) new TappedTransformerElm(x1, y1);
		if (n == "TransLineElm")
			return (ElmBase) new TransLineElm(x1, y1);
		if (n == "RelayElm")
			return (ElmBase) new RelayElm(x1, y1);
		if (n == "MemristorElm")
			return (ElmBase) new MemristorElm(x1, y1);
		if (n == "SparkGapElm")
			return (ElmBase) new SparkGapElm(x1, y1);
		if (n == "ClockElm")
			return (ElmBase) new ClockElm(x1, y1);
		if (n == "AMElm")
			return (ElmBase) new AMElm(x1, y1);
		if (n == "FMElm")
			return (ElmBase) new FMElm(x1, y1);
		if (n == "LampElm")
			return (ElmBase) new LampElm(x1, y1);
		if (n == "PushSwitchElm")
			return (ElmBase) new PushSwitchElm(x1, y1);
		if (n == "OpAmpElm")
			return (ElmBase) new OpAmpElm(x1, y1);
		if (n == "OpAmpSwapElm")
			return (ElmBase) new OpAmpSwapElm(x1, y1);
		if (n == "NMosfetElm" || n == "MosfetElm")
			return (ElmBase) new NMosfetElm(x1, y1);
		if (n == "PMosfetElm")
			return (ElmBase) new PMosfetElm(x1, y1);
		if (n == "NJfetElm" || n == "JfetElm")
			return (ElmBase) new NJfetElm(x1, y1);
		if (n == "PJfetElm")
			return (ElmBase) new PJfetElm(x1, y1);
		if (n == "AnalogSwitchElm")
			return (ElmBase) new AnalogSwitchElm(x1, y1);
		if (n == "AnalogSwitch2Elm")
			return (ElmBase) new AnalogSwitch2Elm(x1, y1);
		if (n == "SchmittElm")
			return (ElmBase) new SchmittElm(x1, y1);
		if (n == "InvertingSchmittElm")
			return (ElmBase) new InvertingSchmittElm(x1, y1);
		if (n == "TriStateElm")
			return (ElmBase) new TriStateElm(x1, y1);
		if (n == "SCRElm")
			return (ElmBase) new SCRElm(x1, y1);
		if (n == "DiacElm")
			return (ElmBase) new DiacElm(x1, y1);
		if (n == "TriacElm")
			return (ElmBase) new TriacElm(x1, y1);
		if (n == "TriodeElm")
			return (ElmBase) new TriodeElm(x1, y1);
		if (n == "VaractorElm")
			return (ElmBase) new VaractorElm(x1, y1);
		if (n == "TunnelDiodeElm")
			return (ElmBase) new TunnelDiodeElm(x1, y1);
		if (n == "CC2Elm")
			return (ElmBase) new CC2Elm(x1, y1);
		if (n == "CC2NegElm")
			return (ElmBase) new CC2NegElm(x1, y1);
		if (n == "InverterElm")
			return (ElmBase) new InverterElm(x1, y1);
		if (n == "NandGateElm")
			return (ElmBase) new NandGateElm(x1, y1);
		if (n == "NorGateElm")
			return (ElmBase) new NorGateElm(x1, y1);
		if (n == "AndGateElm")
			return (ElmBase) new AndGateElm(x1, y1);
		if (n == "OrGateElm")
			return (ElmBase) new OrGateElm(x1, y1);
		if (n == "XorGateElm")
			return (ElmBase) new XorGateElm(x1, y1);
		if (n == "DFlipFlopElm")
			return (ElmBase) new DFlipFlopElm(x1, y1);
		if (n == "JKFlipFlopElm")
			return (ElmBase) new JKFlipFlopElm(x1, y1);
		if (n == "SevenSegElm")
			return (ElmBase) new SevenSegElm(x1, y1);
		if (n == "MultiplexerElm")
			return (ElmBase) new MultiplexerElm(x1, y1);
		if (n == "DeMultiplexerElm")
			return (ElmBase) new DeMultiplexerElm(x1, y1);
		if (n == "SipoShiftElm")
			return (ElmBase) new SipoShiftElm(x1, y1);
		if (n == "PisoShiftElm")
			return (ElmBase) new PisoShiftElm(x1, y1);
		if (n == "PhaseCompElm")
			return (ElmBase) new PhaseCompElm(x1, y1);
		if (n == "CounterElm")
			return (ElmBase) new CounterElm(x1, y1);
		...
		if (n == "DecadeElm" || n == "RingCounterElm")
			return (ElmBase) new RingCounterElm(x1, y1);

		if (n == "TimerElm")
			return (ElmBase) new TimerElm(x1, y1);
		if (n == "DACElm")
			return (ElmBase) new DACElm(x1, y1);
		if (n == "ADCElm")
			return (ElmBase) new ADCElm(x1, y1);
		if (n == "LatchElm")
			return (ElmBase) new LatchElm(x1, y1);
		if (n == "SeqGenElm")
			return (ElmBase) new SeqGenElm(x1, y1);
		if (n == "VCOElm")
			return (ElmBase) new VCOElm(x1, y1);
		if (n == "BoxElm")
			return (ElmBase) new BoxElm(x1, y1);
		if (n == "TextElm")
			return (ElmBase) new TextElm(x1, y1);
		if (n == "TFlipFlopElm")
			return (ElmBase) new TFlipFlopElm(x1, y1);
		if (n == "SevenSegDecoderElm")
			return (ElmBase) new SevenSegDecoderElm(x1, y1);
		if (n == "FullAdderElm")
			return (ElmBase) new FullAdderElm(x1, y1);
		if (n == "HalfAdderElm")
			return (ElmBase) new HalfAdderElm(x1, y1);
		if (n == "MonostableElm")
			return (ElmBase) new MonostableElm(x1, y1);
		if (n == "LabeledNodeElm")
			return (ElmBase) new LabeledNodeElm(x1, y1);

		// if you take out UserDefinedLogicElm, it will break people's saved shortcuts
		if (n == "UserDefinedLogicElm" || n == "CustomLogicElm")
			return (ElmBase) new CustomLogicElm(x1, y1);
		...
		if (n == "DataRecorderElm")
			return (ElmBase) new DataRecorderElm(x1, y1);
		if (n == "AudioOutputElm")
			return (ElmBase) new AudioOutputElm(x1, y1);
		if (n == "NDarlingtonElm" || n == "DarlingtonElm")
			return (ElmBase) new NDarlingtonElm(x1, y1);
		if (n == "PDarlingtonElm")
			return (ElmBase) new PDarlingtonElm(x1, y1);
		if (n == "ComparatorElm")
			return (ElmBase) new ComparatorElm(x1, y1);
		if (n == "OTAElm")
			return (ElmBase) new OTAElm(x1, y1);
		if (n == "NoiseElm")
			return (ElmBase) new NoiseElm(x1, y1);
		if (n == "VCVSElm")
			return (ElmBase) new VCVSElm(x1, y1);
		if (n == "VCCSElm")
			return (ElmBase) new VCCSElm(x1, y1);
		if (n == "CCVSElm")
			return (ElmBase) new CCVSElm(x1, y1);
		if (n == "CCCSElm")
			return (ElmBase) new CCCSElm(x1, y1);
		if (n == "OhmMeterElm")
			return (ElmBase) new OhmMeterElm(x1, y1);
		if (n == "ScopeElm")
			return (ElmBase) new ScopeElm(x1, y1);
		if (n == "FuseElm")
			return (ElmBase) new FuseElm(x1, y1);
		if (n == "LEDArrayElm")
			return (ElmBase) new LEDArrayElm(x1, y1);
		if (n == "CustomTransformerElm")
			return (ElmBase) new CustomTransformerElm(x1, y1);
		if (n == "OptocouplerElm")
			return (ElmBase) new OptocouplerElm(x1, y1);
		if (n == "StopTriggerElm")
			return (ElmBase) new StopTriggerElm(x1, y1);
		if (n == "OpAmpRealElm")
			return (ElmBase) new OpAmpRealElm(x1, y1);
		if (n == "CustomCompositeElm")
			return (ElmBase) new CustomCompositeElm(x1, y1);
		if (n == "AudioInputElm")
			return (ElmBase) new AudioInputElm(x1, y1);
		if (n == "CrystalElm")
			return (ElmBase) new CrystalElm(x1, y1);
		if (n == "SRAMElm")
			return (ElmBase) new SRAMElm(x1, y1);
		if (n == "TimeDelayRelayElm")
			return (ElmBase) new TimeDelayRelayElm(x1, y1);
		if (n == "DCMotorElm")
			return (ElmBase) new DCMotorElm(x1, y1);
		if (n == "LDRElm")
			return (ElmBase) new LDRElm(x1, y1);
		if (n == "ThermistorNTCElm")
			return (ElmBase) new ThermistorNTCElm(x1, y1);
		return null;
	}
	...
	public Canvas getCircuitAsCanvas(boolean print) {
		...
		CustomGraphics g = new CustomGraphics(context);

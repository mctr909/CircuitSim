...
public class ScrollValuePopup extends PopupPanel implements MouseOutHandler, MouseWheelHandler,
	...
	ScrollValuePopup(final int x, final int y, int dy, ElmBase e, CirSim s) {
		super();
		myElm = e;
		deltaY = 0;
		sim = s;
	...
	private void setupValues() {
		...
		for (int i = 0; i < nvalues + 1; i++) {
			if (ElmBase.getShortUnitText(currentvalue, "") == ElmBase.getShortUnitText(values[i], "")) { // match
																												// to an
																												// existing
																												// value
	...
	private void setupLabels() {
		...
		for (int i = 0; i < labMax; i++) {
			...
			if ((thissel + i - 2) < 0 || (thissel + i - 2) >= nvalues)
				...
				labels[i].setText(ElmBase.getShortUnitText(values[thissel + i - 2], ""));

...
public abstract class ElmBase implements Editable {
	...
	ElmBase(int xa, int ya, int xb, int yb, int f) {
		x = xa;
		y = ya;
		x2 = xb;
		y2 = yb;
	...
	void setParentList(Vector<ElmBase> elmList) {
	}
	...
	void draw2Leads(CustomGraphics g) {
		// draw first lead
	...
	void drawDots(CustomGraphics g, Point pa, Point pb, double pos) {
	...
	void drawPosts(CustomGraphics g) {
	...
	void drawHandles(CustomGraphics g, Color c) {
		g.setColor(c);
	 ...
	 * }
	 */
	static void drawPost(CustomGraphics g, Point pt) {
	...
	void drawCenteredText(CustomGraphics g, String s, int x, int y, boolean cx) {
	...
	void drawValues(CustomGraphics g, String s, double hs) {
		if (s == null)
			return;
		g.setFont(unitsFont);
		// FontMetrics fm = g.getFontMetrics();
	...
	void drawCoil(CustomGraphics g, int hs, Point p1, Point p2,
			double v1, double v2) {
	...
	static void drawThickLine(CustomGraphics g, int x, int y, int x2, int y2) {
		g.setLineWidth(3.0);
		g.drawLine(x, y, x2, y2);
		g.setLineWidth(1.0);
	}

	static void drawThickLine(CustomGraphics g, Point pa, Point pb) {
		g.setLineWidth(3.0);
		g.drawLine(pa.x, pa.y, pb.x, pb.y);
		g.setLineWidth(1.0);
	}

	static void drawThickPolygon(CustomGraphics g, int xs[], int ys[], int c) {
		// int i;
		// for (i = 0; i != c-1; i++)
	...
	static void drawThickPolygon(CustomGraphics g, Polygon p) {
		drawThickPolygon(g, p.xpoints, p.ypoints, p.npoints);
	}

	static void drawPolygon(CustomGraphics g, Polygon p) {
	...
	static void drawThickCircle(CustomGraphics g, int cx, int cy, int ri) {
		g.setLineWidth(3.0);
		g.context.beginPath();
	...
	Color getVoltageColor(CustomGraphics g, double volts) {
		if (needsHighlight()) {
			return (selectColor);
		}
		if (!sim.voltsCheckItem.getState()) {
	...
	void setVoltageColor(CustomGraphics g, double volts) {
		g.setColor(getVoltageColor(g, volts));
	}

	// yellow argument is unused, can't remember why it was there
	void setPowerColor(CustomGraphics g, boolean yellow) {
	...
	void setPowerColor(CustomGraphics g, double w0) {
		if (!sim.powerCheckItem.getState())
			return;
		if (needsHighlight()) {
	...
	void setConductanceColor(CustomGraphics g, double w0) {
		w0 *= powerMult;
		// System.out.println(w);
		double w = (w0 < 0) ? -w0 : w0;
		if (w > 1)

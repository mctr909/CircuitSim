...
class ScopePlot {
	...
	ScopePlot(ElmBase e, int u) {
		elm = e;
		units = u;
	}

	ScopePlot(ElmBase e, int u, int v, double manS) {
		elm = e;
		units = u;
		value = v;
		manScale = manS;
	...
	String getUnitText(double v) {
		switch (units) {
			...
			case Scope.UNITS_OHMS:
				return ElmBase.getUnitText(v, CirSim.ohmString);
			case Scope.UNITS_W:
	...
	void assignColor(int count) {
		...
		switch (units) {
			case Scope.UNITS_V:
				color = ElmBase.positiveColor.getHexValue();
				break;
			case Scope.UNITS_A:
...
class Scope {
	...
	void setValue(int val) {
		if (plots.size() > 2 || plots.size() == 0)
			return;
		ElmBase ce = plots.firstElement().elm;
	...
	void drawPlot(CustomGraphics g, ScopePlot plot, boolean allPlotsSameUnits, boolean selected) {
		if (plot.elm == null)
			return;
		int i;
		String col;
	...
	void drawCrosshairsInfo(CustomGraphics g, String[] info, int ct, Boolean drawY) {
		int szw = 0, szh = 15 * ct;
		int i;
		for (i = 0; i != ct; i++) {
	...
	void drawScale(ScopePlot plot, CustomGraphics g) {
		if (!isManualScale()) {
			...
			if (rect.y + rect.height <= textY + 5)
				return;
			double x = 0;
			String hs = "H=" + ElmBase.getUnitText(gridStepX, "s") + "/div";
	...
	void selectY() {
		ElmBase yElm = (plots.size() == 2) ? plots.get(1).elm : null;
